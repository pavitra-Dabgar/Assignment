  ...
  function updateProductCards() {
    ...
    const products =
      (
        productData.categories.find(
          (cat) => cat.category_name === currentCategory
        ) || {}
      ).category_products || [];
    ...
    function updateProductCards() {
        ...
        products.forEach((product) => {
          ...
          // Check for badge_text and set color class
          let badgeColorClass = "";
          if (product.badge_text === "Wedding Special" || product.badge_text === "New season") {
            badgeColorClass = "red-text";
          }
      
          productCard.innerHTML = `
            <img src="${product.image}" alt="${product.title}">
            <div class="offer ${badgeColorClass}">${product.badge_text}</div>
            <h3>${product.title}</h3>
            <p>Vendor: ${product.vendor}</p>
            <p>Price: $${product.price}</p>
            <p>Compare at Price: $${product.compare_at_price}</p>
            <p class="discount ${badgeColorClass}">Discount: ${calculateDiscountPercentage(
              product.price,
    ...
    products.forEach((product) => {
      ...
      productCard.innerHTML = `
  <img src="${product.image}" alt="${product.title}">
  <div>${product.badge_text}</div> <!-- Corrected property name here -->
  <h3>${product.title}</h3>
  <p>Vendor: ${product.vendor}</p>
  <p>Price: $${product.price}</p>
  <p>Compare at Price: $${product.compare_at_price}</p>
  <p>Discount: ${calculateDiscountPercentage(
    product.price,
    product.compare_at_price
  )}% off</p>
  <button class="add-to-cart-button">Add to Cart</button>
`;
  ...
  // Initial update of product cards
  updateProductCards();
